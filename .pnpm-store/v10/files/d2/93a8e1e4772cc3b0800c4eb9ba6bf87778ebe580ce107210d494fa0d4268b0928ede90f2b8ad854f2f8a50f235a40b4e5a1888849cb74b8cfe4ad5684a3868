...
internal interface StorageDao {
    ...
    @Transaction @Query("SELECT `$COLUMN_KEY` FROM $TABLE_NAME") suspend fun getKeys(): List<String>

    @Transaction @Query("DELETE FROM $TABLE_NAME") suspend fun clear()
}

/**
 * Previous version of AsyncStorage is violating the SQL standard (based on bug in SQLite), where
 * PrimaryKey ('key' column) should never be null
 * (https://www.sqlite.org/lang_createtable.html#the_primary_key). Because of that, we cannot reuse
 * the old DB, because ROOM is guarded against that case (won't compile).
 *
 * In order to work around this, two steps are necessary:
 * - Room DB pre-population from the old database file
 ...
 *
 * This migration will happens only once, when developer enable this feature (when DB is still not
 * created).
 */
@Suppress("ClassName")
private object MIGRATION_TO_NEXT : Migration(1, 2) {
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        database.execSQL(
            "CREATE TABLE IF NOT EXISTS `${TABLE_NAME}` (`${COLUMN_KEY}` TEXT NOT NULL, `${COLUMN_VALUE}` TEXT, PRIMARY KEY(`${COLUMN_KEY}`));"
        )
...
fun ReadableArray.toEntryList(): List<Entry> {
    ...
    for (keyValue in this.toArrayList()) {
        require(keyValue is ArrayList<*> && keyValue.size == 2)
        val key = keyValue[0]
